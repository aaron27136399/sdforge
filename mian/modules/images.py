...
# ===== 新增配置项（在文件顶部添加） =====
opts.blur_radius = 24                # 模糊半径（可调参数）
opts.encrypt_original = True        # 是否加密原始数据
opts.encryption_key = "your-32-byte-secret-key-here"  # AES-256密钥（32字节）

def encrypt_data(data: bytes, key: str) -> bytes:
    """AES-CBC加密（含随机IV）"""
    key_bytes = key.ljust(32, '\0')[:32].encode()  # 确保32字节
    iv = get_random_bytes(16)
    cipher = AES.new(key_bytes, AES.MODE_CBC, iv=iv)
    return iv + cipher.encrypt(pad(data, AES.block_size))  # 返回 IV+密文
# ===== 新增配置项（在文件顶部添加） =====	
...
def save_image_with_geninfo(image, geninfo, filename, extension=None, existing_pnginfo=None, pnginfo_section_name='parameters'):
    ...
    if extension.lower() == '.png':
        ...
        # 将原始图像以 JPG 格式保存到内存
        image.save(jpg_buffer, format=image_format, quality=opts.jpeg_quality, lossless=opts.webp_lossless)
        ...
        #     piexif.insert(exif_bytes, jpg_buffer)


        jpg_data = jpg_buffer.getvalue()  # 获取原始 JPG 二进制数据
        # 压缩原始数据（减少加密体积）
        compressed_data = zlib.compress(jpg_data, level=9)

        # AES-256-CBC 加密
        key = opts.encryption_key.ljust(32, '\0')[:32].encode()
        iv = get_random_bytes(16)
        cipher = AES.new(key, AES.MODE_CBC, iv=iv)
        encrypted_data = iv + cipher.encrypt(pad(compressed_data, AES.block_size))
        ...
        new_size = (width // 4, height // 4)  # 面积缩小到 1/16
        
        # 生成模糊缩略图
        blurred_thumbnail = image.resize(new_size)
        blurred_thumbnail = blurred_thumbnail.filter(ImageFilter.GaussianBlur(opts.blur_radius))
        ...
        # 2. 添加加密数据
        existing_pnginfo["EncryptedOriginal"] = encrypted_b64
        pnginfo_data = PngImagePlugin.PngInfo()
        for k, v in existing_pnginfo.items():
            pnginfo_data.add_text(k, str(v))
        
        # 保存模糊缩略图（PNG）
        blurred_thumbnail.save(
            filename,
            format='PNG',
            pnginfo=pnginfo_data,
            optimize=True  # 启用 PNG 优化压缩
        )
